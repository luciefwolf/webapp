...
def get_games():
    ...
    try:
        ...
        for row in cursor: 
            ...
            if game_id not in game_ids: #if not already listed
...
def get_a_specific_game(game_id):
    ...
    try:
        ...
        count = 0
        for row in cursor:
            if count == 0: #if this is the very first step, do basic setup

                images = json.loads(row[12].replace("'", '"'))
                if images['header_image'] == '': #if there is no header image
                    images['header_image'] = 'https://external-content.duckduckgo.com/iu/?u=https%3A%2F%2Fwww.simplystamps.com%2Fmedia%2Fcatalog%2Fproduct%2F5%2F8%2F5802-n-a-stock-stamp-hcb.png&f=1&nofb=1&ipt=4c91608ffabe756cef98c89e32321f03e9ae4c3ab4a92fb4b68453801fd7cf7e&ipo=images'
                game = {'title': row[0],
            ...
            else: #add more developers, publishers, categories, and genres
...
def get_stats():
    ...
    try:
        ...
        # use query results to form final results dictionary
...
def add_args_to_query(args, get_genre_dev_name = False):
    ...
    # adds every applicable argument from the search function to the query and chart title
    ...
    if 'genre_id' in args: 
        ...
        # get name of genre to add to chart_title
    ...
    if 'developer_id' in args: 
        ...
        # get name of developer to add to chart_title
        if get_genre_dev_name:
            try:
                connection = get_connection()
                cursor = connection.cursor()
